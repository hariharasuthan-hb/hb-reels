    ...
    public function dataTable($query)
    {
        ...
        return $dataTable
            ...
            ->editColumn('payment_method', function ($expense) {
                return $expense->readable_payment_method;
            })
            ->editColumn('vendor', function ($expense) {
                return $expense->vendor ?: '—';
            })
            ->editColumn('notes', function ($expense) {
                return $expense->notes ? str($expense->notes)->limit(40) : '—';
            })
            ->addColumn('action', function ($expense) {
                $viewUrl = route('admin.expenses.show', $expense->id);
                $editUrl = route('admin.expenses.edit', $expense->id);
                $deleteUrl = route('admin.expenses.destroy', $expense->id);
                ...
                $html .= '</a>';
                $html .= '<form action="' . $deleteUrl . '" method="POST" class="inline delete-form" data-confirm="true" data-confirm-title="Delete Expense" data-confirm-message="Are you sure you want to delete this expense record? This action cannot be undone.">';
                $html .= csrf_field();
